...
class VerlToolAgentLoop(AgentLoopBase):
    ...
    async def _execute_maestro_skills(self, traj_id: str, search_content: str, extra_fields: Any = None):
        """
        Execute skills using Maestro style skill calling.
        ...
        # Add Maestro skills path to sys.path
        # current_dir = os.path.dirname(os.path.dirname(os.path.dirname(__file__)))
        # maestro_skills_path = os.path.join(current_dir, "skills")
        # if maestro_skills_path not in sys.path:
        #     sys.path.insert(0, maestro_skills_path)

        # # Add Maestro directory to sys.path for imports
        # maestro_path = os.path.join(current_dir, "Maestro")
        # if maestro_path not in sys.path:
        #     sys.path.insert(0, maestro_path)

        try:
            # print("?"*10)
            # Import Maestro modules
            from Maestro.model_skill_orchestrator import request_skill_task, check_llm_name, check_skill_name

            # Parse the search content: Model-Name@@Skill-Name:Your-Input
            if "@@" in search_content and ":" in search_content:
                parts = search_content.split(":", 1)
                if len(parts) == 2:
                    ...
                    if "@@" in model_skill_part:
                        ...
                        # Normalize names using Maestro functions
                        llm_info = check_llm_name(target_llm=model_name)
                        if isinstance(llm_info, tuple) and len(llm_info) >= 3:
                            LLM_NAME, TAU, PORT = llm_info
                        else:
                            LLM_NAME, TAU, PORT = model_name, 1.0, None

                        normalized_skill_name = check_skill_name(target_skill=skill_name)

                        if not LLM_NAME:
                            obs_text = "Model Name Error"
                            done = True
                            is_valid_action = False
                        elif not normalized_skill_name:
                            obs_text = "Skill Name Error"
                            done = True
                            is_valid_action = False
                        else:
                            # Prepare skill task data with extra fields (including images)
                            task_data = (0, LLM_NAME, normalized_skill_name, skill_input,
                                       self.agent_config.tool_server_url, "", None, PORT, extra_fields)

                            # Execute skill in thread pool
                            loop = asyncio.get_event_loop()
                            with ThreadPoolExecutor(max_workers=1) as executor:
                                result = await loop.run_in_executor(executor, request_skill_task, task_data)

                            q_id, response, completion_tokens = result
                            obs_text = response
                            done = True  # Skill execution is complete
                            is_valid_action = True
                    else:
                        obs_text = "Format Error: Missing @@ separator"
                        done = True
                        is_valid_action = False
                else:
                    obs_text = "Format Error: Missing : separator"
                ...
                obs_text = "Format Error: Invalid skill call format"
                done = True
                is_valid_action = False

        except Exception as e:
            logging.error(f"Maestro skill execution failed: {e}", exc_info=True)
            obs_text = f"Skill execution error: {str(e)}"
            done = True
            is_valid_action = False
        ...
        if isinstance(obs_text, str):
            ...
            tool_interact_info['obs'] = str(obs_text)
            tool_interact_info['reward'] = None

        tool_interact_info['obs'] = tool_interact_info['obs'][:MAX_OBS_LENGTH]
        tool_interact_info['reward'] = None
        tool_interact_info['trajectory_id'] = traj_id
        tool_interact_info['done'] = done  # Add done flag
        tool_interact_info['valid_action'] = is_valid_action  # Add valid_action flag
    ...
    async def interact_with_tool_server(
        ...
        # Check if action contains <search> tags (Maestro skill calling format)
        import re
        search_pattern = r'<search>(.*?)</search>'
        search_matches = re.findall(search_pattern, action, re.DOTALL)

        if search_matches:
            # Use Maestro style skill calling
            return await self._execute_maestro_skills(traj_id, search_matches[0], extra_fields)
        else:
            # No <search> tags found, skip Maestro skill calling
            logger.info(f"No <search> tags found in action, skipping Maestro skill call")
            # Return empty tool interaction info
            return {
                'obs': '',
                'reward': None,
                ...
                'done': True,  # Mark as done since no tool was called
                'valid_action': False,  # No valid action since no tool was called
                'finish': not do_action,
                'invalid_reason': 'no_search_tags',
    ...
    async def close_traj_tool_threads(
        ...
        """
            ...
            Skip cleanup in Router-R1 mode since we don't use the original tool server.
        """
        # In Router-R1 mode, we don't use the original tool server, so skip cleanup
        # This prevents connection errors when the tool server is not actually needed
        logger.debug(f"Skipping tool server cleanup for traj_id={request_id} (Router-R1 mode)")
        return 
    ...
    async def run(self, sampling_params: dict[str, Any], **kwargs) -> AgentLoopOutput:
        ...
        image_paths = multi_modal_data.get("image_paths", [])
        ...
        for step in range(max_turns + 1):
            ...
            available_length = max(max_response_length - len(running_prompt_ids) + len(prompt_ids), 8192)
            ...
            # logger.warning(f"Turn {step}: available_length={available_length}, max_tokens_for_this_turn={max_tokens_for_this_turn}")
            # agent_sampling_params["max_new_tokens"] = max_tokens_for_this_turn # for sglang
            with simple_timer("generate_sequences", metrics):
                import random
                # if random.randint(1, 32) == 1:
                #     running_prompt = self.tokenizer.decode(running_prompt_ids, skip_special_tokens=False)
                #     print(f"running_prompt: {running_prompt}")
                ...
                if output.text.strip() == "":
                    import random   
                    if random.randint(1, 16) == 1 and step != 0:
                        running_prompt = self.tokenizer.decode(running_prompt_ids)
                        print(f"step: {step}   running_prompt:   {running_prompt}")
                        logger.warning(f"Turn {step}: Generated empty response for traj_id={request_id}. prompt_ids length: {len(running_prompt_ids)}. agent_sampling_params: {agent_sampling_params}")
            ...
            # New truncation logic based on step and content
            truncated = False
            original_gen_text = gen_text

            if step == 0:
                # Step 0: 保留 <search> 内容，但限制 <search> 之前的文字长度为 200 tokens
                if "<search>" in gen_text and "</search>" in gen_text:
                    # 1. 拆分字符串
                    # prefix: <search> 之前的内容
                    # search_content: <search>...</search> 的完整内容
                    search_start_idx = gen_text.find("<search>")
                    last_search_end_pos = gen_text.rfind("</search>") + len("</search>")
                    
                    prefix_text = gen_text[:search_start_idx]
                    search_content = gen_text[search_start_idx:last_search_end_pos]
                    
                    # 2. 对 prefix 进行 token 截断 (限长 200)
                    prefix_tokens = self.tokenizer.encode(prefix_text)
                    if len(prefix_tokens) > 200:
                        # 取最后 200 个 token（通常思考过程的结尾更接近搜索意图）
                        # 如果你想取开头的 200 个，改用 [:200]
                        prefix_tokens = prefix_tokens[-200:] 
                        prefix_text = self.tokenizer.decode(prefix_tokens)
                    
                    # 3. 重新组装：[截断后的前缀] + [完整的search内容] + [结束符]
                    gen_text = prefix_text + search_content + "<|im_end|>"
                    
                    # 4. 重新计算 gen_tokens
                    new_gen_tokens = self.tokenizer.encode(gen_text)
                    
                    # 5. 更新 gen_ids 和 gen_logprobs，保持原有逻辑对齐
                    if len(new_gen_tokens) <= len(gen_ids):
                        original_length = len(gen_ids)
                        # 取新序列的前 N-1 个，加上原始序列的最后一个 (通常是 EOS)
                        gen_ids = new_gen_tokens[:-1] + gen_ids[-1:]
                        gen_logprobs = gen_logprobs[:len(new_gen_tokens)-1] + gen_logprobs[-1:]
                        
                        truncated = True
                        logger.debug(f"Step 0: Prefix truncated to 200. Total tokens: {original_length} -> {len(gen_ids)}")
                    else:
                        # 如果重组后的长度超过了原始长度，回退处理
                ...
                # Step != 0
                # First, if </think> exists, remove everything before and including </think>
                if "</think>" in gen_text:
                    think_end_pos = gen_text.find("</think>") + len("</think>")
                    gen_text = gen_text[think_end_pos:]
                if "</think>" == gen_text:
                    gen_text = []
                    gen_ids = []
                    gen_logprobs = []
                ...
                if gen_text != []:
                    ...
                    if step == 1:
                        # Not last step: look for <search> or <answer>
                        if "<search>" in gen_text and "</search>" in gen_text:
                            # Extract the last <search>...</search> block
                            import re
                            search_matches = re.findall(r'<search>.*?</search>', gen_text, re.DOTALL)
                            if search_matches:
                                last_search = search_matches[-1]
                                gen_text = last_search + "<|im_end|>"

                                gen_tokens = self.tokenizer.encode(gen_text)
                                if len(gen_tokens) <= len(gen_ids):
                                    original_length = len(gen_ids)
                                    gen_ids = gen_ids[-len(gen_tokens):]
                                    gen_logprobs = gen_logprobs[-len(gen_tokens):]
                                    truncated = True
                                    logger.debug(f"Step {step}: Extracted last <search> block: {original_length} -> {len(gen_ids)} tokens")
                                else:
                                    gen_text = original_gen_text

                        elif "<answer>" in gen_text and "</answer>" in gen_text:
                            # Extract the <answer>...</answer> block
                            import re
                            answer_matches = re.findall(r'<answer>.*?</answer>', gen_text, re.DOTALL)
                            if answer_matches:
                                last_answer = answer_matches[-1]
                                gen_text = last_answer + "<|im_end|>"
                                ...
                                if len(gen_tokens) <= len(gen_ids):
                                    ...
                                    gen_logprobs = gen_logprobs[-len(gen_tokens):]
                                    truncated = True
                                    logger.debug(f"Step {step}: Extracted <answer> block: {original_length} -> {len(gen_ids)} tokens")
                                else:
                                    gen_text = original_gen_text
                            else:
                                # No valid <answer> block found, set to empty and continue
                                gen_text = ""
                                gen_ids = []
                                gen_logprobs = []
                                continue
                        else:
                            # Neither <search> nor <answer> found, set to empty and continue
                            gen_text = ""
                            gen_ids = []
                            gen_logprobs = []
                            continue
                        ...
                        # Last step: only extract <answer> if exists
                        if "<answer>" in gen_text and "</answer>" in gen_text:
                            ...
                            if answer_matches:
                                ...
                                if len(gen_tokens) <= len(gen_ids):
                                    ...
                                    gen_logprobs = gen_logprobs[-len(gen_tokens):]
                                    truncated = True
                                    logger.debug(f"Last step: Extracted <answer> block: {original_length} -> {len(gen_ids)} tokens")
                                else:
                                    gen_text = original_gen_text
                        elif "<search>" in gen_text and "</search>" in gen_text:
                            # Extract the last <search>...</search> block
                            import re
                            search_matches = re.findall(r'<search>.*?</search>', gen_text, re.DOTALL)
                            if search_matches:
            ...
            # Remove all <think> and </think> tags from the final text
            # import re
            # if '<think>' in gen_text or '</think>' in gen_text:
            #     # Remove all think tags and their content
            #     gen_text_no_think = re.sub(r'<think>.*?</think>', '', gen_text, flags=re.DOTALL)
            #     # Also remove any remaining isolated think tags
            #     gen_text_no_think = gen_text_no_think.replace('<think>', '').replace('</think>', '')
            #     # Re-encode to get new token ids
            #     no_think_tokens = self.tokenizer.encode(gen_text_no_think)
            #     if len(no_think_tokens) <= len(gen_ids):
            #         original_length = len(gen_ids)
            #         gen_ids = gen_ids[-len(no_think_tokens):]
            #         gen_logprobs = gen_logprobs[-len(no_think_tokens):]
            #         gen_text = gen_text_no_think
            #         logger.debug(f"Removed all <think> tags: {original_length} -> {len(gen_ids)} tokens")

            turncated_gen_text = self.tokenizer.decode(gen_ids, skip_special_tokens=False)
            # import random   
            # if random.randint(1, 4) == 1 and step != 0:
            #     print(f"step: {step}  turncated_gen_text: {turncated_gen_text}")
            ...
            # Check if final answer is generated - this should stop the trajectory
            if '</answer>' in gen_text:
                is_last_step = True
                stats_dict["is_traj_finished"] = True
                traj_stop_reason = "final_answer_generated"
                break
            ...
            #         if action_stop_token == '</search>' and action_stop_token in stop_reason:
            #             # do action
            #             do_action = True
            #             action_text = (gen_text.split(action_stop_token)[0] + action_stop_token)
            #             break
            if '<search>' in turncated_gen_text and '<answer>' not in turncated_gen_text:
                do_action = True
                action_text = (turncated_gen_text.split('</search>')[0] + '</search>')
            # print(f"do_action: {do_action}  is_last_step: {is_last_step}")
            if do_action and not is_last_step:
                ...
                # Add <information> tags around obs_text
                obs_text = f"<|im_start|>user<information>{obs_text}</information><|im_end|>"
                if tool_results.get('image', None):
                    ...
                    if max_obs_length < len(obs_token_ids):
                        if self.agent_config.truncate_obs_side == 'left':
                            obs_token_ids = obs_token_ids[-max_obs_length:]
                            # Add <information> start tag after truncation
                            truncated_text = "<|im_start|>user<information>" + self.tokenizer.decode(obs_token_ids)
                            obs_token_ids = self.tokenizer.encode(truncated_text)
                        elif self.agent_config.truncate_obs_side == 'right':
                            obs_token_ids = obs_token_ids[:max_obs_length]
                            # Add <information> end tag after truncation
                            truncated_text = self.tokenizer.decode(obs_token_ids) + "</information><|im_end|>"
                            obs_token_ids = self.tokenizer.encode(truncated_text)
                            ...
                            # For middle truncation, we need to reconstruct the full tags
                            truncated_text = "<|im_start|>user<information>" + self.tokenizer.decode(obs_token_ids) + "</information><|im_end|>"
                            obs_token_ids = self.tokenizer.encode(truncated_text)
                        else:
        ...
        if self.agent_config.mask_void_traj:
            ...
            has_answer = "\\boxed" in response_text or "final_answer(" in response_text or "</answer>" in response_text
