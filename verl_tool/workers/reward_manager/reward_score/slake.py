...
def compute_score(solution_str, ground_truth, format_score=0.0, score=1., cost_coe=0.0, api_cost=0.0, state="train", reward_metric="f1", return_answer=False):
    """
    Compute score for SLAKE questions.
    ...
    Args:
        ...
        api_cost: API cost
        state: "train" or "val"
        reward_metric: Not used for slake (always binary 0/1)
    ...
    try:
        ...
        if not extracted_answer:
            ...
            if not target:
                ...
                target_answer = str(target[0]).strip()
                target_lower = target_answer.lower()

                # Check if the answer is correct based on target type
                is_correct = False

                if target_lower in ['yes', 'no']:
                    # For yes/no questions, target must be a substring of the extracted answer
                    extracted_lower = extracted_answer.lower()
                    is_correct = target_lower in extracted_lower
                else:
                    # For other answers, allow bidirectional substring matching
                    extracted_lower = extracted_answer.lower()
        ...
        if random.randint(1, 64) == 1:
            print(f"[debug slake] solution_str: {solution_str}")
            print(f"[debug slake] target: {target_answer if 'target_answer' in locals() else 'N/A'}")
            print(f"[debug slake] answer: {answer}")
            if answer is not None and 'target_answer' in locals():
                print(f"[debug slake] is_correct: {accuracy_score}")
            print(f"[debug slake] accuracy_score: {accuracy_score}")
        ...
        print(f"Error in compute_score for slake: {e}")
...
def compute_score_slake(solution_str, ground_truth, format_score=0.0, **kwargs):
    """
