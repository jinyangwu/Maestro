...
def is_numeric_only(text):
    """Check if the text contains only numeric characters and common number symbols."""
    # Remove spaces, commas, periods, and other common number formatting
    cleaned = re.sub(r'[ ,.]', '', text)
    # Check if remaining characters are all digits or common number symbols
...
def compute_score(solution_str, ground_truth, format_score=0.0, score=1., cost_coe=0.0, api_cost=0.0, state="train", reward_metric="f1", return_answer=False):
    """
    Compute score for ChartQA questions.
    ...
    Args:
        ...
        api_cost: API cost
        state: "train" or "val"
        reward_metric: Not used for chartqa (always binary 0/1)
    ...
    try:
        ...
        if not extracted_answer:
            ...
            # Get target answer from ground truth (only first element)
            target = ground_truth
            if not target:
                accuracy_score = 0.0
                answer = extracted_answer
            else:
                target_answer = target[0]

                # Check if the answer is correct based on content type
                is_correct = False

                if is_numeric_only(target_answer):
                    try:
                        pred_val = float(extracted_answer.strip())
                        target_val = float(target_answer.strip())

                        # 保留两位小数后比较
                        is_correct = round(pred_val, 2) == round(target_val, 2)

                    except ValueError:
                        # 如果无法转成数字，则判错
                        is_correct = False
                else:
                    # If target contains letters, allow substring matching
                    extracted_lower = extracted_answer.lower()
                    target_lower = target_answer.lower()
                    is_correct = (extracted_lower in target_lower) or (target_lower in extracted_lower)
        ...
        # if True:
            print(f"[debug chartqa] solution_str: {solution_str}")
            print(f"[debug chartqa] target: {target_answer if 'target_answer' in locals() else 'N/A'}")
            print(f"[debug chartqa] answer: {answer}")
            if answer is not None and 'target_answer' in locals():
                print(f"[debug chartqa] is_correct: {accuracy_score}")
            print(f"[debug chartqa] accuracy_score: {accuracy_score}")
        ...
        print(f"Error in compute_score for chartqa: {e}")
...
def compute_score_chartqa(solution_str, ground_truth, format_score=0.0, **kwargs):
    """
    ...
    # # Test case 2: Text answer with substring matching
    # print("\nTest case 2 - Text answer (substring matching):")
    # print("-" * 50)
    # solution_str = "<search>The chart shows that the highest value is Red</search><answer>The chart shows that the highest value is Red</answer>"
    # ground_truth = ["The red bar is the highest"]
    # train_metric, train_cost, train_reward = compute_score(
    ...
    # print("\nTest case 3 - Wrong answer:")
    # print("-" * 30)
    # solution_str = "The answer is 25"
    # ground_truth = ["42"]
    # train_metric, train_cost, train_reward = compute_score(
    #     solution_str=solution_str,
    #     ground_truth=ground_truth,
    #     state="train",
    #     format_score=0.5,  # 格式部分正确
    #     api_cost=0.0,
    #     cost_coe=0.0,
    # )
    # print(f"Solution string: {solution_str}")
    ...
    # # Test case 4: No answer extracted
    # print("\nTest case 4 - No answer extracted:")
    # print("-" * 35)
    # solution_str = "This is just some text without a clear answer."
    # ground_truth = ["42"]
    # train_metric, train_cost, train_reward = compute_score(
    #     solution_str=solution_str,
    #     ground_truth=ground_truth,
